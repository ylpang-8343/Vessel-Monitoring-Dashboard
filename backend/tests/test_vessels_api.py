...
def test_create_vessel_without_destination():
...
def test_create_vessel_with_destination():
    resp = client.post(
        "/api/vessels",
...
def test_list_vessels_search_by_name_imo_or_destination():
    client.post(
        "/api/vessels",
...
def test_history_not_found_for_unknown_imo():
    resp = client.get("/api/vessels/0000000/history")
    assert resp.status_code == 404


def test_history_returns_empty_timeline_for_new_vessel():
